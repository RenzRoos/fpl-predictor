...
from utils.regressor_params import get_regressor_params
...
def predict_gameweek(data: pd.DataFrame, players_df: pd.DataFrame, gw: int, FEATURES: list, target: str, N_RUNS: int) -> pd.DataFrame:
    ...
    for pid, name in zip(players_df['id'], players_df['web_name']):
        if((players_df.loc[players_df['id'] == pid, 'element_type'] == 1).any() and (target == 'goals_scored' or target == 'assists')) or (
            (players_df.loc[players_df['id'] == pid, 'element_type'] != 1).any() and target == 'saves'):
            results.append({
                "player_id": int(pid),
                "player_name": name,
                "round": int(gw),
                f"predicted_{target}": 0
            })
            continue

        print(f"Predicting {target} for {name} (ID: {pid}) for GW{gw}...")
        ...
        for _ in range(N_RUNS):
            model = RandomForestRegressor(**get_regressor_params(target), random_state=(random.randint(1, 1_000_000)))
        ...
        results.append({
            ...
            "player_name": name,
            "round": int(gw),
            f"predicted_{target}": mean_pred
        })

    df_out = pd.DataFrame(results)
    col = f"predicted_{target}"
    if df_out.empty or col not in df_out.columns:
        return df_out.reset_index(drop=True)
    return df_out.sort_values(col, ascending=False).reset_index(drop=True)