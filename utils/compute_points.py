...
def compute_predicted_points(preds: pd.DataFrame, players: pd.DataFrame, team_gc: pd.Series ) -> pd.Series:
    ...
    # -----------------------------------------------------------
    total = mins_pts + goal_pts + ast_pts + yc_pts + bonus_pts + save_pts + cs_pts + gc_penalty

    all_preds = pd.DataFrame()
    all_preds["predicted_points"] = total
    all_preds["predicted_mins"] = mins_pts
    all_preds["predicted_goals"] = goal_pts
    all_preds["predicted_assists"] = ast_pts
    all_preds["predicted_yellow_cards"] = yc_pts
    all_preds["predicted_bonus"] = bonus_pts
    all_preds["predicted_saves"] = save_pts
    all_preds["predicted_clean_sheet"] = cs_pts
    all_preds["predicted_goals_conceded"] = gc_penalty
